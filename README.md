...
This repository contains the complete Phase 0 through Phase 9 V1: official source ingestion, institution reconciliation, legal-entity resolution, deterministic technical-role classification, processed metrics, a DuckDB query layer, the evidence explorer, positive-only ICE OPT evidence, prioritized E-Verify lookups, reviewed official-policy extraction, versioned nullable evidence-strength scores, five-organization comparison, publication quality gates, scheduled refreshes, private release packaging, and operational recovery. It preserves immutable raw evidence, the raw-name to legal-entity to parent-organization chain, versioned SOC/title and score decisions, retrieval evidence, review queues, explicit coverage, and partial-period labels.
...
- `src/sponsor_intel/quality/` computes visible, publication-blocking V1 gates; `src/sponsor_intel/releases/` creates private release assets only after those gates pass.
